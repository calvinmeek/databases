<?php
	...
	$queryD = pg_query($conn, "SELECT manager_name,open_hour,close_hour,url FROM restaurant R, location L INNER JOIN (SELECT itemID, MAX(price) price FROM menuItem GROUP BY itemID) M ON R.name = '$testString4' AND R.restaurantID = L.restaurantID AND R.restaurantID = M.restaurantID");
